...
export class ShiftsMethods {
  ...
  async getShifts() {
    const shiftRows = await this.workTimeDatabase.execute(queries['select-shifts'], []);
    const shiftDurationRows = await this.workTimeDatabase.execute(queries['select-shift-durations'], []);
